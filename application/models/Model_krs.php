...
class Model_krs extends CI_Model {
    ...
    public function get_list_krs($id_prodi, $semester) {
        $query = $this->db->select('j.id_jadwal as id, m.kode_matkul as kode, m.nama, m.sks, m.semester, d.nama as dosen, j.hari, j.pukul as waktu, j.id_ruangan as ruangan')
            ...
            ->where(['j.id_tahun' => $this->session->tahun, 'm.id_prodi' => $id_prodi, 'k.semester_mhs' => $semester])->get();
    ...
    public function get_mk($nim, $semester) {
        $this->db->from('ak_matkul m')->join('ak_jadwal j', 'm.id_matkul = j.id_matkul')->join('ak_krs k', 'j.id_jadwal = k.id_jadwal')
            ->where(['k.nim' => $nim, 'k.semester_mhs' => $semester]);
    ...
    public function get_krs($nim = '', $type = '') {
        if ($nim === '') {
            $query = $this->db->select('j.id_jadwal as id, j.id_tahun, k.id_krs, m.kode_matkul as kode, m.nama, m.sks, m.semester, d.nama as dosen, j.hari, j.pukul as waktu, j.id_ruangan as ruangan, k.status, m.semester')
                ...
                ->where('j.id_tahun', $this->session->tahun)->order_by('j.id_tahun', 'ASC')->get();
        } else {
            $query = $this->db->select('j.id_jadwal as id, j.id_tahun, k.id_krs, m.kode_matkul as kode, m.nama, m.sks, m.semester, d.nama as dosen, j.hari, j.pukul as waktu, j.id_ruangan as ruangan, k.status, m.semester')
                ->from('ak_dosen d')->join('ak_matkul m', 'd.nik = m.nik_dosen')->join('ak_jadwal j', 'm.id_matkul = j.id_matkul')->join('ak_krs k', 'j.id_jadwal = k.id_jadwal')
                ->where(['j.id_tahun' => $this->session->tahun, 'k.nim' => $nim])->order_by('j.id_tahun', 'ASC')->get();
        }
    ...
    public function get_krs_smt($nim, $semester, $status = '') {
        if ($status === '') {
            $query = $this->db->select('j.id_jadwal as id, j.id_tahun, m.kode_matkul as kode, m.nama, m.sks, m.semester, d.nama as dosen, j.hari, j.pukul as waktu, j.id_ruangan as ruangan, k.status, k.nilai')
                ->from('ak_dosen d')->join('ak_matkul m', 'd.nik = m.nik_dosen')->join('ak_jadwal j', 'm.id_matkul = j.id_matkul')->join('ak_krs k', 'j.id_jadwal = k.id_jadwal')
                ->where(['k.nim' => $nim, 'k.semester_mhs' => $semester])->get();
        } else {
            $query = $this->db->select('j.id_jadwal as id, j.id_tahun, m.kode_matkul as kode, m.nama, m.sks, m.semester, d.nama as dosen, j.hari, j.pukul as waktu, j.id_ruangan as ruangan, k.status, k.nilai')
                ->from('ak_dosen d')->join('ak_matkul m', 'd.nik = m.nik_dosen')->join('ak_jadwal j', 'm.id_matkul = j.id_matkul')->join('ak_krs k', 'j.id_jadwal = k.id_jadwal')
                ->where(['k.status' => $status, 'k.nim' => $nim, 'k.semester_mhs' => $semester])->get();
        }
    ...
    public function get_sks($nim, $semester = '') {
        if ($semester === '') {
            ...
            $query = $this->db->select('m.sks, k.nilai')->from('ak_matkul m')->join('ak_jadwal j', 'm.id_matkul = j.id_matkul')->join('ak_krs k', 'j.id_jadwal = k.id_jadwal')
                ->where(['k.status' => 'Y', 'k.nim' => $nim, 'k.semester_mhs' => $semester])->get();
        }
    ...
    public function set_krs($input) {
        $mahasiswa = $this->get_db('ak_mahasiswa', ['nim' => $this->session->id]);
        $data = [
            'nim' => $mahasiswa['nim'],
            'semester_mhs' => $mahasiswa['semester'],
        ];
